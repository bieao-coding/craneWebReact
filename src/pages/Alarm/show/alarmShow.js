...
class AlarmShow extends Component{
  ...
  transformToTree = (data) => {
    ...
    data.forEach((item)=>{
      ...
      content.total += alarmTotal.total;
      content.overLoad += alarmTotal.overLoad;
      content.collision += alarmTotal.collision;
      content.limit += alarmTotal.limit;
      content.area += alarmTotal.area;
      content.windSpeed += alarmTotal.windSpeed;
      alarmTotal.children = dateList.map((value)=>{
        index++;
        return {...value,...{index:index.toString(),id:item.id}}
      });
      total.push(alarmTotal);
    });
    ...
    this.chartsData.forEach((item,index)=>{
      if(item.children && item.children.length){
        ...
        chart.scale({
          ...
          value:{
            alias:formatMessage({ id: 'app.statistics.alarm-number' })
          }
        });
        chart.axis('value', {
          title: {
  ...
  onPageChange = (page, pageSize) =>{
    ...
    this.setState({
      ...
      this.clickNumber(this.alarmParams,1);
    });
  };
  /*点击数字跳转*/
  clickNumber = ({alarmCode,date,id},type) => {
    this.alarmParams = {alarmCode,date,id};
  ...
  render(){
    ...
    return (
      <Row className='all-height p-l-10' id='content'>
