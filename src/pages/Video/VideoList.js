...
@connect(({ videoList,user, loading }) => ({
  videoList,
...
class VideoList extends Component {
  ...
  /*请求事件*/
  getList = () => {
    const { dispatch,videoList } = this.props;
    const {pageNumber = 0, pageSize = 10} = videoList;
    const params = {...this.state.params,...{pageNumber,pageSize}};
    this.setState({params});
    dispatch({
      type: 'videoList/fetch',
      payload:params
    });
  };
  /*页码改变事件*/
  ...
  /*新增*/
  add = () => {
    router.push('/device/video/list/add');
  };
  /*查看绑定记录*/
  bindRecord = (record) => {
    if(!record || !record.sn) return;
    router.push({
      pathname:'/device/video/list/record',
      state:{sn:record.sn}
    });
  };
  /*选择变化*/
  ...
  render() {
    const {videoList,loading} = this.props;
    const {list,total} = videoList;
