...
export default class AnimationTwo{
  radius = 0;
  x = 0;
  y = 0;
  oldRadius = 0;
  ...
  /*画图*/
  draw({x, y, arm,balanceArm, radius, rotation,craneType, name, app}) {
    ...
    for(let i = 0; i < 360/Math.PI; i++){
      if(!(i%2)){
        ...
        arc.lineStyle(circleLine, 0x00ff99, opcity);
    ...
    richText.x = x - 15;
    richText.y = y - 25;
    app.stage.addChild(richText, this.crane,this.dash,this.car);
    this.x = x;
    this.y = y;
    this.radius = radius;
