...
class PropertyCompany extends Component {
  ...
  columns = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <Fragment>
          {this.props.auth.propertyCompany_edit ? (
            <Fragment>
          ...
          {this.props.auth.propertyCompany_addNext ? (
            <Fragment>
  ...
  addAndEdit = (type,record) => {
    ...
    if(!type){
      ...
      obj.businessLevel = 1;
      router.push({
        pathname:'/base/company/propertyCompany/add',
        state:obj
      });
    }else if (type === 1){
      ...
      router.push({
        pathname:'/base/company/propertyCompany/addNext',
        state:obj
      });
    }else{
      obj.companyId = record.companyId;
      router.push({
        pathname:'/base/company/propertyCompany/edit',
        state:obj
      });
    }
  ...
  onChange = (pagination) => {
    ...
    this.props.dispatch({
      type: 'company/setPage',
      payload:{propertyPageNumber:current}
    })
  };
  /*显示等级*/
  showLevel = (level) =>{
  ...
  render() {
    ...
    const {propertyPageNumber = 0} = this.props.company;
    ...
    return (
      <div>
          ...
          <Row gutter={{ md:8,sm:8,xs: 8 }} >
            ...
            <Col xxl = {19} xl = {18} lg ={17} xs = {16}>
              <Row type="flex" justify="end">
                {this.props.auth.propertyCompany_add ? (
        ...
        <Table
          ...
          loading={loading}
          pagination = {{current:propertyPageNumber}}
