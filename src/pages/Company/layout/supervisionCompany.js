...
import {transPingTreeToChildren,recursionTopToObj} from '@/utils/utils'
import {Divider,Popconfirm,message,Form,Card,Row,Col,Button,Input,Tag,Table} from 'antd';
...
class SupervisionCompany extends Component {
  state = {
    list:[],
    filterList:[],
    rowKey:'companyId',
  ...
  addAndEdit = (type,record) => {
    ...
    if(!type){
      ...
      obj.businessLevel = 1;
      router.push({
        pathname:'/base/company/supervisionCompany/add',
        state:obj
      });
    }else if (type === 1){
      ...
      router.push({
        pathname:'/base/company/supervisionCompany/addNext',
        state:obj
      });
    }else{
      obj.companyId = record.companyId;
      router.push({
        pathname:'/base/company/supervisionCompany/edit',
        state:obj
      });
    }
  ...
  onChange = (pagination) => {
    ...
    this.props.dispatch({
      type: 'company/setPage',
      payload:{supervisionPageNumber:current}
    })
  };
  /*显示等级*/
  showLevel = (level) =>{
  ...
  render() {
    ...
    const {supervisionPageNumber = 0} = this.props.company;
    ...
    return (
      <div>
          ...
          <Row gutter={{ md:8,sm:8,xs: 8 }} >
            ...
            <Col xxl = {19} xl = {18} lg ={17} xs = {16}>
              <Row type="flex" justify="end">
                {this.props.auth.supervisionCompany_add ? (
        ...
        <Table
          ...
          loading={loading}
          pagination = {{current:supervisionPageNumber}}
...
export default SupervisionCompany;
