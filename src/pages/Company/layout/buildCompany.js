...
class BuildCompany extends Component {
  state = {
    filterList:[],
    rowKey:'companyId',
  ...
  columns = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <Fragment>
          {this.props.auth.buildCompany_edit ? (
            <Fragment>
          ...
          {this.props.auth.buildCompany_addNext ? (
            <Fragment>
  ...
  addAndEdit = (type,record) => {
    ...
    if(!type){
      ...
      obj.businessLevel = 1;
      router.push({
        pathname:'/base/company/buildCompany/add',
        state:obj
      });
    }else if (type === 1){
      ...
      router.push({
        pathname:'/base/company/buildCompany/addNext',
        state:obj
      });
    }else{
      obj.companyId = record.companyId;
      router.push({
        pathname:'/base/company/buildCompany/edit',
        state:obj
      });
    }
  ...
  onChange = (pagination) => {
    ...
    this.props.dispatch({
      type: 'company/setPage',
      payload:{buildPageNumber:current}
    })
  };
  /*显示等级*/
  showLevel = (level) =>{
  ...
  render() {
    ...
    const {buildPageNumber = 0} = this.props.company;
    ...
    return (
      <div>
          ...
          <Row gutter={{ md:8,sm:8,xs: 8 }} >
            ...
            <Col xxl = {19} xl = {18} lg ={17} xs = {16}>
              <Row type="flex" justify="end">
                {this.props.auth.buildCompany_add ? (
        ...
        <Table
          ...
          loading={loading}
          pagination = {{current:buildPageNumber}}
