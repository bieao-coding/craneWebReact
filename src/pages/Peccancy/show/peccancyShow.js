...
class PeccancyShow extends Component{
  chartWidth = 0;
  chartsData = [];
  state = {
    activeKey:'0',
  ...
  transformToTree = (data) => {
    ...
    data.forEach((item)=>{
      ...
      content.total += peccancyTotal.total;
      content.overLoad += peccancyTotal.overLoad;
      content.collision += peccancyTotal.collision;
      content.limit += peccancyTotal.limit;
      content.area += peccancyTotal.area;
      content.windSpeed += peccancyTotal.windSpeed;
      peccancyTotal.children = dateList.map((value)=>{
        index++;
        return {...value,...{index:index.toString(),id:item.id}}
      });
      total.push(peccancyTotal);
    });
    ...
    this.chartsData.forEach((item,index)=>{
      if(item.children && item.children.length){
        ...
        chart.scale({
          ...
          value:{
            alias:formatMessage({ id: 'app.statistics.peccancy-number' })
          }
        });
        chart.axis('value', {
          title: {
  ...
  onPageChange = (page, pageSize) =>{
    ...
    this.setState({
      ...
      this.clickNumber(this.peccancyParams,1);
    });
  };
  /*点击数字跳转*/
  clickNumber = ({alarmCode,date,id},type) => {
    this.peccancyParams = {alarmCode,date,id};
    ...
    this.props.dispatch({
      type: 'peccancy/getPeccancyDetails',
      payload:newparams,
      callback:(res)=>{
        if(res) {
