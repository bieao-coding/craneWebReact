...
import {Card, Row, Col, Tree, Button, Input} from 'antd';
import {transPingTreeToChildren,resMessage,compare,recursionTopToObj } from '@/utils/utils'
...
class WorkCompanyTree extends React.Component {
  state={
    userId:null,
    oldData:[],
    treeData:[],
  ...
  getList = () => {
    ...
    dispatch({
      ...
      callback:(res)=>{
        const newList = res.orgWorkListVo.filter((item)=>item.businessLevel < 20).sort(compare('orgId',0));
  ...
  requestParentSelect(params){
    ...
    const newParams = {businessType:1,userId:type ? parentUserId : userId};
    dispatch({
  ...
  requestSelect(userId,list){
    const { dispatch } = this.props;
    const params = {businessType:1,userId:userId};
    dispatch({
      type: 'platformUser/getSelectOrganization',
      payload:params,
      callback:(res)=>{
  ...
  saveAuth = () => {
    ...
    dispatch({
      ...
      payload:{userId:this.userInfo.userId,orgIdSet:checkItem,businessType:1},
      callback:(res)=>{
        resMessage(res);
  ...
  render(){
    const {treeData,expandedKeys,autoExpandParent,checkItem,saveLoading,placeHolder} = this.state;
    return (
      <div>
...
export default WorkCompanyTree;
