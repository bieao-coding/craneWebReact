...
class buildCompanyTree extends React.Component {
  state={
    userId:null,
    oldData:[],
    treeData:[],
  ...
  getList = () => {
    ...
    dispatch({
      ...
      callback:(res)=>{
        const newList = res.orgBuildListVo.filter((item)=>item.businessLevel < 20).sort(compare('orgId',0));
  ...
  requestParentSelect(params){
    ...
    const newParams = {businessType:2,userId:type ? parentUserId : userId};
    dispatch({
  ...
  requestSelect(userId,list){
    const { dispatch } = this.props;
    const params = {businessType:2,userId:userId};
    dispatch({
      type: 'platformUser/getSelectOrganization',
      payload:params,
      callback:(res)=>{
  ...
  saveAuth = () => {
    ...
    dispatch({
      ...
      payload:{userId:this.userInfo.userId,orgIdSet:checkItem,businessType:2},
      callback:(res)=>{
        resMessage(res);
...
export default buildCompanyTree;
