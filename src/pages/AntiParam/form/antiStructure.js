...
@Form.create()
class AntiStructure extends Component {
  state = {
    readLoading:false,
    writeLoading:false,
    text:'',
  ...
  readParams = () =>{
    ...
    sendCmd({cmd:'ExtAntiCollision',vo:{},craneId:this.state.id,rwStatus:'R'},this.showData);
  };
  /*设置参数*/
  handleSubmit = (e) =>{
  ...
  render() {
    ...
    const formRules = {
      slewWarning3:{
        rules:[{required: true,validator:validateSelf(/^((\d+(.\d+)?)|0)$/,0,180)}],
        initialValue:this.state.params.slewWarning3
      },
      slewWarning:{
        rules:[{required: true,validator:validateSelf(/^((\d+(.\d+)?)|0)$/,0,180)}],
        initialValue:this.state.params.slewWarning
      },
      slewAlarm:{
        rules:[{required: true, validator:validateSelf(/^((\d+(.\d+)?)|0)$/,0,180)}],
        initialValue:this.state.params.slewAlarm
      },
      minSlewWarning:{
        rules:[{required: true, validator:validateSelf(/^((\d+(.\d+)?)|0)$/,0,180)}],
        initialValue:this.state.params.minSlewWarning
      },
      breakSlewWarning:{
        rules:[{required: true, validator:validateSelf(/^((\d+(.\d+)?)|0)$/,0,180)}],
        initialValue:this.state.params.breakSlewWarning
      },
      breakDelay:{
        rules:[{required: true,validator:validateSelf(/^\d+$/,0,30)}],
        initialValue:this.state.params.breakDelay
      },
      radiusWarning:{
        rules:[{required: true, validator:validateSelf(/^((\d+(.\d+)?)|0)$/,1,20)}],
        initialValue:this.state.params.radiusWarning
      },
      radiusAlarm:{
        rules:[{required: true, validator:validateSelf(/^((\d+(.\d+)?)|0)$/,0,20)}],
        initialValue:this.state.params.radiusAlarm
      },
      minSlewSpeed:{
        rules:[{required: true, validator:validateSelf(/^((\d+(.\d+)?)|0)$/,0,5)}],
        initialValue:this.state.params.minSlewSpeed
      }
    };
    ...
    return (
      <div>
        ...
        <Form onSubmit={this.handleSubmit}>
          <FormItem
            {...formItemLayout}
            label={formatMessage({id:'app.device.slewWarning3'})}
          >
            {getFieldDecorator('slewWarning3', formRules.slewWarning3)(
              <Input />
            )}
          </FormItem>
          <FormItem
            {...formItemLayout}
            label={formatMessage({id:'app.device.slewWarning'})}
          >
            {getFieldDecorator('slewWarning', formRules.slewWarning)(
              <Input />
            )}
          </FormItem>
          <FormItem
            {...formItemLayout}
            label={formatMessage({id:'app.device.slewAlarm'})}
          >
            {getFieldDecorator('slewAlarm', formRules.slewAlarm)(
              <Input />
            )}
          </FormItem>
          <FormItem
            {...formItemLayout}
            label={formatMessage({id:'app.device.minSlewWarning'})}
          >
            {getFieldDecorator('minSlewWarning', formRules.minSlewWarning)(
              <Input />
            )}
          </FormItem>
          <FormItem
            {...formItemLayout}
            label={formatMessage({id:'app.device.breakSlewWarning'})}
          >
            {getFieldDecorator('breakSlewWarning', formRules.breakSlewWarning)(
              <Input />
            )}
          </FormItem>
          <FormItem
            {...formItemLayout}
            label={formatMessage({id:'app.device.breakDelay'})}
          >
            {getFieldDecorator('breakDelay', formRules.breakDelay)(
              <Input />
            )}
          </FormItem>
          <FormItem
            {...formItemLayout}
            label={formatMessage({id:'app.device.radiusWarning'})}
          >
            {getFieldDecorator('radiusWarning', formRules.radiusWarning)(
              <Input />
            )}
          </FormItem>
          <FormItem
            {...formItemLayout}
            label={formatMessage({id:'app.device.radiusAlarm'})}
          >
            {getFieldDecorator('radiusAlarm', formRules.radiusAlarm)(
              <Input />
            )}
          </FormItem>
          <FormItem
            {...formItemLayout}
            label={formatMessage({id:'app.device.minSlewSpeed'})}
          >
            {getFieldDecorator('minSlewSpeed', formRules.minSlewSpeed)(
              <Input />
            )}
          </FormItem>
        </Form>
